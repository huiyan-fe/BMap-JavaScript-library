...
(function () {
    ...
    /**
     * 栅格路况图层（从JSAPI 3.0抽离）
     * 支持实时路况和流量预测（传入predictDate参数）
     * 在4.0环境下替代不支持predictDate的矢量TrafficLayer
     */
    function RasterTrafficLayer(opts) {
        this._opts = opts || {};
        this._tileUrl = 'https://itsmap2.baidu.com/traffic/';
        // 参数验证
        if (this._opts.predictDate) {
            if (this._opts.predictDate.weekday < 1 || this._opts.predictDate.weekday > 7) {
                this._opts.predictDate.weekday = 1;
            }
    ...
    RasterTrafficLayer.prototype = new BMap.TileLayer({});
    RasterTrafficLayer.prototype.isTransparentPng = function () {
        return true;
    };
    RasterTrafficLayer.prototype.getTilesUrl = function (point, level) {
        var pDate = '';
        if (this._opts.predictDate) {
            pDate = "HistoryService?"
                + "day=" + (this._opts.predictDate.weekday - 1)
                + "&hour=" + this._opts.predictDate.hour
                + "&t=" + new Date().getTime()
                + "&";
        } else {
            pDate = "TrafficTileService?"
                + "time=" + new Date().getTime()
                + "&label=web2D&v=016&";
        }
        var url = this._tileUrl + pDate
            + "level=" + level
            + "&x=" + point.x
            + "&y=" + point.y
            + "&scaler=2";
     ...
     * 对外暴露栅格路况图层，供用户独立使用
     */
    BMapLib.RasterTrafficLayer = RasterTrafficLayer;
    ...
    TrafficControl.prototype._popUpDiv = function (me, btn) {
        ...
        function initialize() {
            ...
            // 获取服务器时间
            var curTimeUrl = 'https://itsmap2.baidu.com/traffic/GetCurrentTime?callback=BMapLib.TrafficControl.getTime&';
    ...
    TrafficControl.prototype.showTraffic = function (predictDate) {
        ...
        if(predictDate){
            ...
            trafficLayer = new RasterTrafficLayer(predictDate);
        }else{
            trafficLayer = new RasterTrafficLayer();
        }
        this.bShow = true;
        if (baidu.isMobile()) {
